...
```bash
openocd -s scripts -f connect_jlink_jtag.cfg
```

Можно использовать несколько ```.cfg``` файлов
```bash
openocd -f connect_jlink_jtag.cfg -f ../generic/mass_erase.cfg
```

Подключение и выполнение следующих команд
```bash
openocd -f connect_jlink_jtag.cfg -c "flash info 0" -c "exit"
```
...
Программирование файла ```program.bin``` с адреса 0x80000000 верификации и выход
```bash
program program.bin verify reset 0x80000000
```
...
Сохранение дампа памяти в файл ```dump.bin``` размером 1кБ, начиная с адреса ```0x80000000```
```bash
dump_image dump.bin 0x80000000 0x400
```

Загрузка бинарного файла ```dump.bin``` в память, начиная с адреса ```0x80000000```
```bash
load_image dump.bin 0x80000000
```
